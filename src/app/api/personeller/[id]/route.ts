...
export async function DELETE(
  ...
  if (adminProfile?.role !== 'admin') return NextResponse.json({ error: 'Sadece admin personel silebilir' }, { status: 403 })

  const { data: targetProfile } = await supabaseAuth.from('profiles').select('id, user_id, role').eq('id', id).single()
  if (!targetProfile) return NextResponse.json({ error: 'Personel bulunamadı' }, { status: 404 })
  if (targetProfile.role !== 'personel') return NextResponse.json({ error: 'Sadece personel silinebilir' }, { status: 400 })
