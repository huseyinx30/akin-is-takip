...
export function OnayBekleyenHarcamalar({ ekipId }: { ekipId: string }) {
  const [expenses, setExpenses] = useState<any[]>([])
  const supabase = createClient()

  useEffect(() => {
    supabase.from('team_expenses').select('*').eq('ekip_id', ekipId).eq('status', 'beklemede').order('expense_date', { ascending: false })
  ...
  const handleApprove = async (id: string) => {
    ...
    await supabase.from('team_expenses').update({ status: 'onaylandi', approved_by: profile?.id, approved_at: new Date().toISOString() }).eq('id', id)
    setExpenses((prev) => prev.filter((e) => e.id !== id))
  }

  const handleReject = async (id: string) => {
    await supabase.from('team_expenses').update({ status: 'reddedildi' }).eq('id', id)
    setExpenses((prev) => prev.filter((e) => e.id !== id))
  }
