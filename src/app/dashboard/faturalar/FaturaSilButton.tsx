...
export function FaturaSilButton({ faturaId, faturaNo }: { faturaId: string; faturaNo: string }) {
  ...
  const handleDelete = async () => {
    setLoading(true)
    const res = await fetch(`/api/faturalar/${faturaId}`, { method: 'DELETE' })
    setLoading(false)
      ...
      {showConfirm && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
          <div className="bg-white rounded-xl shadow-xl max-w-md w-full p-6">
            ...
            <p className="text-[#555] text-sm mb-4">Fatura <strong>{faturaNo}</strong> silmek istediğinize emin misiniz? Bu işlem geri alınamaz.</p>
